...
2026-07-06: **拆书工作区搭建 + 3 本书 ingest**。新建 `拆书/` 目录（选题/、输出/、视频/、bgm/），更新 book-cracker / book-cracker-video / book-trending 三个 skill 的输出路径指向项目内目录。fork 仓库到 ghost1030/claude-obsidian，remote 配置完成。**ingest 3 本书**：[[小狗钱钱]]（财商启蒙，梦想储蓄罐+成功日记+会下金蛋的鹅）、[[情感勒索]]（FOG 三角，情感操纵识别与边界建立）、[[金钱心理学]]（行为金融学，复利、运气与风险、守富、时间自由）。wiki/sources/ 新增 3 个页面，manifest/index/log/hot 同步更新。下一步：用 book-cracker 为已 ingest 的书籍生成拆书输出，或用 book-cracker-video 生成视频。

2026-07-02: **电子书知识库启动**。用户导入 5 本电子书（EPUB 格式）到 `.raw/` 目录，已用 pandoc 提取并创建 wiki 页面：[[认知觉醒]]（周岭，自我提升）、[[显微镜下的大明]]（马伯庸，历史纪实）、[[DeepSeek原理与项目实战]]（未来智能实验室，AI 技术）、[[大众哲学]]（艾思奇，哲学入门）、[[娱乐至死]]（尼尔·波兹曼，媒介批判）。同时创建了变现路径规划 [[未来的可能]]，涵盖自媒体、社群、课程、咨询、SaaS 等 10 条变现路径。wiki/index.md 已更新（Total pages: 40, Sources: 7）。**下一步**：用户挑选 3 本书的内容进行深度提炼，测试小红书/知乎发布反馈。
